...
public class FlxWire_Impl implements FlxWire{
	...
	{
		//System.out.println("constructor start: " + wireName);
		try
		{
			...
			/*this.srcWireEnd.setCoordX(this.dest.getCoordX()+5.0*xEndOffset);
			this.srcWireEnd.setCoordY(this.dest.getCoordY()+5.0*yEndOffset);
			this.destWireEnd.setCoordX(this.dest.getCoordX()-5.0*xEndOffset);
			this.destWireEnd.setCoordY(this.dest.getCoordY()-5.0*yEndOffset);*/
		...
		//System.out.println("constructor end: " + wireName);
	}
	...
	public FlxWire_Impl(String wireName, double srcX, double srcY, double destX, double destY, String parentEffectName)
	{
		...
		try
		{
			...
			if(this.wireName.contains(">"))
			{
				...
				/*System.out.println("FlxWire connections: " 
						+ srcX + ":" + srcY +
						">" + destX + ":" + destY);*/
	...
	public FlxWire_Impl(JsonValue processConnectionJson, String parentEffectName)
	{
		...
		try
		{
			...
			{
				//System.out.println("getting newer wire");
			...
			/*System.out.println("FlxWire connections: " + this.wireName + ": "
					+ srcX + "," + srcY +
					">" + destX + "," + destY);*/
			this.wire = new Line();
			this.wireGroup = new Group();
	...
	{
		/*this.wire = new Line(this.src.getX().doubleValue(),src.getY().doubleValue(),
				dest.getX().doubleValue(),dest.getY().doubleValue());
		this.wire.startXProperty().bind(this.src.getX());
		this.wire.startYProperty().bind(this.src.getY());
		this.wire.endXProperty().bind(this.dest.getX());
		this.wire.endYProperty().bind(this.dest.getY());*/
	...
	public JsonValue getProcessConnectionData() {
		...
		/*JsonObjectBuilder src = Json.createObjectBuilder();
		...
		catch(Exception e)
		{
			...
			//processConnectionDataBuilt = (JsonObject) Json.createObjectBuilder().add("name","error").build();
		}
		/*JsonObjectBuilder processConnectionData = Json.createObjectBuilder();
		JsonObjectBuilder src = Json.createObjectBuilder();
		JsonObjectBuilder dest = Json.createObjectBuilder();*/
	...
	public String getProcessConnectionString() {
		if(this.debugStatements) System.out.println("getting process connection data: " + this.wireName);
		String processConnectionString = "";//Json.createObjectBuilder();
		//JsonObject processConnectionDataBuilt = null;
		...
		try
		{
			...
			/*processConnectionData.add("parentEffect", this.parentEffectName);
			processConnectionData.add("src", this.src.getConnectionString());
			processConnectionData.add("dest", this.dest.getConnectionData());
			processConnectionDataBuilt = processConnectionData.build();*/
